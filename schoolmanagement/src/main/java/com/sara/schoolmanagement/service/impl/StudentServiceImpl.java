...
public class StudentServiceImpl implements StudentService 
{
	...
	{
		logger.debug("Entry for Create Admission for - {}",admissionRequest.getFirstName());
		...
		try
		{
			...
			if(null != persistedStudent && persistedStudent.getStudentId() >0 && persistedStudent.getAdmissionNo()>0)
			{
				logger.debug("Admission Generated and persisted for - {} with admission no - {}",persistedStudent.getFirstName(),persistedStudent.getAdmissionNo());
				...
				return utils.frameResponse(CommonConstants.SUCCESS_RESPONSE_CODE, CommonConstants.STATUS_SUCCESS, CommonConstants.ADMISSION_SUCCESS, StringUtils.EMPTY);
			}
			else
				return utils.frameResponse(CommonConstants.FAILURE_RESPONSE_CODE, CommonConstants.STATUS_FAILURE, CommonConstants.ADMISSION_FAILED, StringUtils.EMPTY);
		}
		catch (Exception e)
		{
			logger.debug("Exception on Create Admission - {}",e.getMessage());
			return utils.frameResponse(CommonConstants.FAILURE_RESPONSE_CODE, CommonConstants.STATUS_FAILURE, "Unable to Create Admission for - "+admissionRequest.getFirstName(), StringUtils.EMPTY);
		}
	...
	{
		List<StudentDetail> students = studentRepo.findByClass(standard,division);
